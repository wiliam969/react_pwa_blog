...
class Cuztom_Field_Image extends Cuztom_Field
{
	...
	function _output( $value )
	{
		...
		$output .= sprintf( '<input id="upload-image-button" type="button" class="button js-cuztom-upload" data-cuztom-media-type="image"' . ( ! empty($this->args["preview_size"] ) ? ' data-cuztom-media-preview-size="' . ( is_array($this->args["preview_size"] ) ? esc_attr( json_encode($this->args["preview_size"] ) ) : $this->args["preview_size"] ) . '"' : '') . ' value="%s" />', __( 'Bild auswählen', 'cuztom' ) );
		$output .= ( ! empty( $value ) ? sprintf( '<a href="#" class="js-cuztom-remove-media cuztom-remove-media">%s</a>', __( 'Entferne dieses Bild', 'cuztom' ) ) : '' );
